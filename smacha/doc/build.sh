...
# MOUNT_POINT: The absolute path of the current package, either locally, or in
#              a GitLab runner container.
...
sphinx-build -W -b html /$CI_PROJECT_NAME /$CI_PROJECT_NAME/public_native_build

# Test build with ROS-version of Sphinx command so that it is generated same as ros.org
rosdoc_lite -o /$CI_PROJECT_NAME/public_build /$CI_PROJECT_NAME

# Copy the html to the public folder
mkdir -p /$CI_PROJECT_NAME/public
cp -r /$CI_PROJECT_NAME/public_build/html/* /$CI_PROJECT_NAME/public/

# Run HTML tests on generated build output to check for 404 errors, etc
# htmlproofer /$CI_PROJECT_NAME/public --only-4xx --check-html --file-ignore /$CI_PROJECT_NAME/public/genindex.html,/$CI_PROJECT_NAME/public/search.html,/$CI_PROJECT_NAME/public/index-msg.html --alt-ignore '/.*/' --url-ignore '#'
